...
Remove global env watchlists (`NAUTILUS_WATCH_SYMBOLS`, default monitor/autonomous-agent watchlists). Poll and alert only symbols declared by autonomous agents (`aa_*`) or `/agent` sessions (`ws_{session_id}`) via the unified watch registry under `reports/hub/_data/watches/`.
...
| 5 | Monitor/autonomous-agent/options jobs derive symbols from agents + registry |
