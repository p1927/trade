...
**Hub as center (channel):** All market-data reads go through `trade_integrations.hub_capture.channel` with `FreshnessPolicy` (`LIVE`, `NORMAL`, `WATCH`); never call OpenAlgo REST or SDK for quotes/chain/history outside `trade_integrations/openalgo/`. For registered entities (v1: NIFTY), read hub first (`latest.json` + capture parquet), fetch vendor only when stale, write-through a capture copy on every live fetch. OpenAlgo fetchers (`fetch_option_chain`, `fetch_openalgo_quote`), MCP `get_options_browse`, TradingAgents OpenAlgo vendors, and index macro PCR read-back use the channel; vendors are fallback only. WATCH TTL: `OPENALGO_WATCH_QUOTE_TTL_SECONDS` (default 5s).
