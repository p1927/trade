...
**Hub as center (channel):** All market-data reads go through `trade_integrations.hub_capture.channel` with `FreshnessPolicy` (`LIVE`, `NORMAL`, `WATCH`); never call OpenAlgo REST or SDK for quotes/chain/history outside `trade_integrations/openalgo/`. For registered entities (NIFTY, BANKNIFTY, RELIANCE, TCS, INDIAVIX by default), read hub first (`latest.json` + capture parquet + OHLCV bars), fetch vendor only when stale, write-through on live fetch. Nautilus watch uses WATCH policy + optional OpenAlgo WebSocket (`OPENALGO_WS_ENABLED=1`) to seed L1 cache. WATCH TTL: `OPENALGO_WATCH_QUOTE_TTL_SECONDS` (default 5s).
