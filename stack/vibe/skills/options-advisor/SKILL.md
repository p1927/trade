...
- **User is not in the loop** — no confirmation per order; `execute_autonomous_basket` allowed.
...
- **Always** `record_autonomous_decision` (ENTER/EXIT/HOLD/SKIP).
- **Tools:** all options-advisor MCP tools + autonomous-agent tools (`start/stop_autonomous_agents`, `get_autonomous_market_feedback`, `get_autonomous_agent_status`, `execute_autonomous_basket`, `record_autonomous_decision`).
- Near close: evaluate day P&L vs goal; flatten or hold as you judge best.

Start: `propose_autonomous_agent(ticker, budget_inr=..., goal=...)`.

**Stop (removes agent cron jobs):** `stop_autonomous_agents` MCP — agent or user. HTTP: `POST /autonomous-agents/{agent_id}/stop`. Legacy pre-migration cron jobs are cleaned up automatically on stop.
...
| `propose_autonomous_agent` | **Start active intraday paper session** (agent-driven) |
| `stop_autonomous_agents` | Stop paper session |
| `get_autonomous_market_feedback` | **Every turn** — market changes, alerts, deltas |
| `get_autonomous_agent_status` | Session P&L, positions, funds, decisions |
| `execute_autonomous_basket` | Enter paper trade from widget after research |
| `record_autonomous_decision` | Log ENTER/EXIT/HOLD/SKIP each turn |
