"""Append-only trade outcomes for autonomous agent calibration and reflection."""
...
def load_ledger() -> pd.DataFrame:
    ...
    if csv_path.is_file():
        try:
            ...
            return pd.DataFrame()
    legacy_path = get_hub_dir() / _LEGACY_LEDGER_REL
    legacy_csv = legacy_path.with_suffix(".csv")
    if legacy_path.is_file() or legacy_csv.is_file():
        try:
