"""Tests for auto-paper session P&L baseline."""
...
def test_us_symbol_skips_openalgo_pnl() -> None:
    ...
    assert "does not apply" in block.get("note", "")


def test_budget_baseline_not_sandbox_cash() -> None:
    session = {"budget_inr": 20_000}
    with patch("trade_integrations.auto_paper.openalgo_client.OpenAlgoClient") as mock_cls:
